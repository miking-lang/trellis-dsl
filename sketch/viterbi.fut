...
-- NOTE: similar to Slack-exmaple, but using a 3mer model
...
let viterbi_forward [m] (predecessors : [nstates][]state_t) (transp : state_t -> state_t -> prob_t)
                        ...
                        (chi1 : [nstates]prob_t)
                        : {chi : [nstates]prob_t, zeta : [m][nstates]state_t} =
  ...
  loop {chi, zeta} = {chi = chi1, zeta = zeta} for i < m do
    ...
    let x = signal[i] in
    let new_zeta = map (\s -> max_state (log_prob_from s) predecessors[state.to_i64 s]) zeta[i] in
    let new_chi : [nstates]prob_t =
      map2 (\s pre -> log_prob_from s (state.i64 pre) + outp s x) new_zeta (indices new_zeta)
    in
...
let in_max (x : state_t) (y : state_t) : bool =
  (x / 16) == (y / 16) && (x % 16) + 1 == 16 && (y % 16) + 1 == 16

let in_from_max (x : state_t) (y : state_t) : bool =
  (x / 16) == (y / 16) && (x % 16) + 1 == 16 && (y % 16) + 1 == 15

let in_down (x : state_t) (y : state_t) : bool =
  (x / 16) == (y / 16) && (x % 16) + 1 == ((y % 16) + 1) - 1
...
entry viterbi [n][m]
  ...
  (table_trans1 : [64][64]prob_t) (table_trans2 : [16]prob_t) (table_gamma : prob_t)
  (predecessors : [nstates][]state_t) (input_signals : [n][m]obs_t)
  ...
  -- TODO(larshum, 2023-12-21): For the below part, we would probably want to
  -- only run on a few inputs at once, to prevent us from running out of memory.
  -- Somehow, we need to be aware of the available GPU memory in this case...
  map
    (\signal ->
      ...
      let bacc = loop bacc for i < nbatches do
        ...
        let out = main_viterbi predecessors transp initp outp batch in
